...
class QLearningTest(testClasses.TestCase):
    ...
    def __init__(self, question, testDict):
        ...
        if sys.platform == 'win32':
            ...
            _, question_name, test_name = testDict['test_out_file'].split('/')
        self.additionalExperiences = 0
        if 'additionalExperiences' in testDict: 
            additionalExperiences = [int(x) for x in testDict['additionalExperiences'].split(' ')]
            self.numsExperiencesForDisplay += additionalExperiences
            self.additionalExperiences = sum(additionalExperiences)
        if maxPreExperiences < numExperiences:
            self.numsExperiencesForDisplay.append(numExperiences)
        self.experiences = Experiences(test_name.split('.')[0], additional_exp=self.additionalExperiences)
