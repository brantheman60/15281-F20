...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeActionFromQValues(self, state):
        ...
        # Get the action/s w/ the highest q-value
        actions = []
        q_value = self.computeValueFromQValues(state)
        for action in legalActions:
          if self.getQValue(state, action) == q_value:
    ...
    def getAction(self, state):
        ...
        if util.flipCoin(self.epsilon):
          ...
          return self.computeActionFromQValues(state) # choose optimal action
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def getQValue(self, state, action):
        ...
        for f in featureVector: # do the dot product!
          q_value += self.weights[f] * featureVector[f]
    ...
    def update(self, state, action, nextState, reward):
        ...
        # difference = r + zeta * max[Q(s',a')] - Q(s,a)
        featureVector = self.featExtractor.getFeatures(state, action)
        # note that self.getValue(nextState) will return max of self.getQValue(nextState, nextAction)
